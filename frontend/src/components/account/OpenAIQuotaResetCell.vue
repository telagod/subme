  ...
  <div v-if="visible" class="space-y-1">
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      ...
      <Button
        ...
        variant="ghost"
        size="sm"
        class="h-auto gap-0.5 rounded px-1.5 py-0.5 text-[10px] font-medium text-primary"
      ...
      <Button
        ...
        variant="ghost"
        size="sm"
        class="h-auto gap-0.5 rounded px-1.5 py-0.5 text-[10px] font-medium text-amber-600 hover:bg-amber-500/10 dark:text-amber-400"
    ...
    <!--
      Reset is a destructive upstream action: it permanently consumes one
      rate-limit-reset credit and resets the current window on chatgpt.com.
      Use the shadcn-vue Dialog (alert-dialog is not registered locally) as
      a confirm modal before firing the POST.
    -->
    <Dialog v-model:open="confirmOpen">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{{ t('admin.accounts.openaiQuotaReset.resetConfirmTitle') }}</DialogTitle>
          <DialogDescription>
            {{ t('admin.accounts.openaiQuotaReset.resetConfirm', { count: availableResetCount }) }}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button variant="outline" size="sm" :disabled="resetting" @click="confirmOpen = false">
            {{ t('common.cancel') }}
          </Button>
          <Button
            variant="default"
