...
describe('admin RiskControlView', () => {
  ...
  it('shows pre-block synchronous moderation metrics separately from worker queue', async () => {
    ...
    expect(syncCard.classes()).toEqual(expect.arrayContaining(['rounded-lg', 'border', 'bg-card']))
    expect(apiKeyLoadCard.classes()).toEqual(expect.arrayContaining(['rounded-lg', 'border', 'bg-card']))
