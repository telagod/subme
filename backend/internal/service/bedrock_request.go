...
// bedrockContextManagementBetaToken 用于 body sanitize：当请求未携带该 beta 时，
// 同步把 body 内的 context_management 字段剥掉，避免 Bedrock 因字段无对应 beta 而报错。
// 与白名单互补：白名单决定 token 是否透传，sanitize 决定字段是否伴随。
