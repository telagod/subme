...
// marshalAnthropicSystemTextBlockWithCacheControl emits a system text block with a
// caller-supplied cache_control value (already decoded into a map). A nil
// cacheControl omits the cache_control field entirely.
...
// rewriteSystemForNonClaudeCodeWithPrompt 在 rewriteSystemForNonClaudeCode 的基础上
// 仅替换第三个（扩展提示词）block 的内容，保留旧的两段静态默认。
...
// claudeOAuthSystemPromptBlockConfig 描述单个可配置 system block。
// 当 enabled 为 nil 时视为开启；text 支持 {billing_header}/{cc_version}/{fp}/
// {claude_code_system_prompt}/{claude_code_expansion_prompt} 占位符。
...
// parseClaudeOAuthSystemPromptBlocksConfig 解析配置：支持纯数组或带 blocks 包裹。
...
// decodeClaudeOAuthSystemPromptCacheControl 把 cache_control JSON 还原为可注入对象：
// null/false/缺失 → nil；true → 默认 ephemeral 5m；object → 直接使用。
...
// expandClaudeOAuthSystemPromptTextTemplate 解析占位符。空文本直接返回空串，调用方据此跳过。
...
// defaultClaudeOAuthSystemPromptBlockConfig 返回内置 3-block 默认配置，
// 行为与历史硬编码完全一致（billing 无 cache_control、cc 身份块无 cache_control、扩展块带 ephemeral 5m）。
...
// buildClaudeOAuthSystemPromptBlocksJSON 根据配置（空 → 默认）构造 system 数组里的每个 block。
...
// ValidateClaudeOAuthSystemPromptBlocksConfig 在管理员保存时校验配置合法性。
...
// rewriteSystemForNonClaudeCodeWithPromptBlocks 是真正干活的实现：根据可配置 blocks 注入
// system 数组，原始 system prompt 被搬迁到 messages 头部的 user/assistant 对里。
func rewriteSystemForNonClaudeCodeWithPromptBlocks(body []byte, system any, expansionPrompt string, blocksConfig string) []byte {
	...
	// 2. 构造 system 数组。默认对齐真实 Claude Code CLI 的 3-block 形态：
	...
	//    [2] 工具无关的通用提示词扩充 block（默认带 cache_control 作为稳定缓存断点）
	//
...
// claudeOAuthSystemPromptInjectionSettings 暴露 Claude OAuth mimic 路径用到的可配置
// system block 设置；当 setting service 未注入时退化为内置默认（开启 + 空覆写）。
