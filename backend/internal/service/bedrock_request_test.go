...
func TestPrepareBedrockRequestBodyWithTokens_ContextManagementRequiresSupportedBeta(t *testing.T) {
	...
	t.Run("keeps fine-grained-tool-streaming beta token", func(t *testing.T) {
		input := `{"messages":[{"role":"user","content":"hi"}],"max_tokens":100}`

		result, err := PrepareBedrockRequestBodyWithTokens(
			[]byte(input),
			modelID,
			[]string{"fine-grained-tool-streaming-2025-05-14"},
			false,
		)
		require.NoError(t, err)

		assert.Equal(t, []string{"fine-grained-tool-streaming-2025-05-14"}, bedrockAnthropicBetaNames(result))
	})
}
