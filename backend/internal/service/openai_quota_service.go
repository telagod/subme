...
func (s *OpenAIQuotaService) QueryUsage(ctx context.Context, accountID int64) (*OpenAIQuotaUsage, error) {
	...
	if !resp.IsSuccessState() {
		...
		body := truncateOpenAIQuotaBody(resp.String(), 240)
...
// generateOpenAIQuotaRedeemRequestID produces a UUID-v4-shaped string without
// pulling in a new dependency. ChatGPT uses this as an idempotency key for the
// consume call.
...
// mapOpenAIQuotaUpstreamStatus collapses upstream HTTP statuses into a stable
// set we surface from the admin handler. 4xx upstream errors are surfaced as
// 502 (BadGateway) so callers can distinguish "your input is bad" (400) from
...
// truncateOpenAIQuotaBody safely truncates a response body for log/error
// surfaces, leaving an ellipsis marker if cut.
func truncateOpenAIQuotaBody(s string, max int) string {
	if max <= 0 || len(s) <= max {
		return s
	}
	return s[:max] + "..."
