...
// LoginOrRegisterOAuthWithTokenPairAndPromoCode 与 LoginOrRegisterOAuthWithTokenPair
// 行为一致，并仅在新用户注册时消费 promoCode。既有用户登录时 promoCode 被忽略，
// 避免重复消费造成的用户体验灾难。
...
// ApplyOAuthSignupPromoCode 暴露给 handler 层的 helper，仅在 pending-session
// 重生成新用户路径（createPendingOAuthAccount）使用——既有 LoginOrRegisterOAuth*
// 系列自己内部走 applyOAuthSignupPromoCode，不需要外部调用。
...
// applyOAuthSignupPromoCode 在 promo 功能启用时为新注册的 OAuth 用户消费 promo 码。
// 既有用户登录路径不应调用此函数（不消费 promo）。
