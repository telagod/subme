...
func TestSchedulerOutboxRepositoryDeleteConsumedUpToDefaultsLimit(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()

	repo := &schedulerOutboxRepository{db: db}
	mock.ExpectExec(`WITH doomed AS`).
		WithArgs(int64(7), schedulerOutboxDefaultCleanSize).
		WillReturnResult(sqlmock.NewResult(0, 0))

	deleted, err := repo.DeleteConsumedUpTo(context.Background(), 7, 0)
