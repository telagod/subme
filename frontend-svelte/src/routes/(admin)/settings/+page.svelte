...
<section class="space-y-5" data-page="admin-settings">
	<header class="flex items-center justify-between">
		...
		<div class="flex items-center gap-2">
			<button
				...
				class="inline-flex h-9 items-center justify-center rounded-md border border-input bg-background px-4 text-sm hover:bg-accent disabled:opacity-50"
				onclick={handleReset}
	...
	<nav class="flex gap-1 border-b border-border" aria-label="settings tabs">
		{#each settingsTabs as tab (tab.id)}
			<button
				...
				data-tab-id={tab.id}
				class="-mb-px inline-flex h-9 items-center border-b-2 px-3 text-sm transition-colors {activeTabId ===
				tab.id
			...
			>
				...
				{#if tab.placeholder}
					<span class="ml-1 text-[10px] text-muted-foreground">(soon)</span>
				{/if}
			</button>
		{/each}
	</nav>
	...
	{:else if activeTab?.placeholder}
		<div class="rounded-lg border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
			{$_('common.comingSoon', { default: 'Coming soon' })}
		</div>
	{:else}
		<div class="space-y-4">
