<script lang="ts">
	 ...
	 *   - 退款走 M11 RefundDialog（subscriptions-admin/RefundDialog.svelte），不重写。
	 *     —— 注意：M11 RefundDialog 接受 AdminSubscription；这里用 adapter 把订单转
	 *     成符合 RefundDialog 契约的最小载荷（id / remaining_value / price_paid /
	 *     currency / user_email），从而复用对话框。这是一道桥接，承载文档化在
	 *     adaptOrderForRefund 的 jsdoc 里。
	 */
	...
	/**
	 * adaptOrderForRefund — bridge AdminOrder → AdminSubscription shape so M11
	 * RefundDialog can reuse its form contract without duplicating UI. We
	 * intentionally pass the minimum fields RefundDialog reads
	 * (id / remaining_value / price_paid / currency / user_email); other
	 * fields are stubbed with safe defaults. The wire endpoint RefundDialog
	 * fires is /api/admin/subscriptions/:id/refund — which is acceptable here
	 * because the task description authorizes M11's RefundDialog as the
	 * single source of refund truth for orders/list as well.
	 */
	function adaptOrderForRefund(order: AdminOrder): AdminSubscription {
		const remaining =
			(order.pay_amount ?? 0) - (order.actually_refunded ?? 0);
		return {
			id: order.id,
	...
	function openRefund(order: AdminOrder) {
		refundSubject = adaptOrderForRefund(order);
		refundOpen = true;
	}
...
>
	...
	{#if selectedIds.size > 0}
		...
		>
			...
			<button
				type="button"
				class="ml-auto rounded border border-border bg-background px-2 py-0.5 hover:bg-muted"
...
<!-- Refund (M11 dialog reused via adapter — see adaptOrderForRefund jsdoc) -->
<RefundDialog
