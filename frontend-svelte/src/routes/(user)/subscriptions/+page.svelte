<script lang="ts">
	...
	async function loadCurrent() {
		...
		try {
			...
			showError(
				$_('user.subscriptions.failedToLoad', { default: 'Failed to load subscriptions' })
			);
		} finally {
...
<section class="space-y-6" data-testid="subscriptions-page">
		...
		<div class="space-y-1">
			...
			<p class="text-sm text-muted-foreground">
				{$_('user.subscriptions.pageSubtitle', {
					default: 'Manage your active subscriptions and view plan history.'
				})}
			</p>
		</div>
	...
	{:else if currentError && !current}
		...
		>
			<span>{$_('user.subscriptions.failedToLoad', { default: 'Failed to load subscriptions' })}</span>
			<Button
				variant="outline"
				size="sm"
		...
		>
			...
			<div class="space-y-1">
				<h2 class="text-base font-semibold text-foreground">
					{$_('user.subscriptions.emptyTitle', { default: 'No active subscriptions' })}
				</h2>
				<p class="max-w-sm text-sm text-muted-foreground">
					{$_('user.subscriptions.emptyDescription', {
						default: 'Choose a plan to use the API beyond the free quota.'
					})}
				</p>
			</div>
			<Button
	...
	<section class="space-y-3" data-testid="subscriptions-history">
		...
		{:else if history.length === 0}
			...
			>
				{$_('user.subscriptions.historyEmpty', { default: 'No subscription history.' })}
			</div>
		{:else}
			<div
				...
				<table class="w-full text-sm" data-testid="subscriptions-history-table">
					<thead>
						...
						>
							...
							<th class="px-4 py-2 text-left font-medium">
								{$_('user.subscriptions.colStatus', { default: 'State' })}
							</th>
							<th class="px-4 py-2 text-left font-medium">
