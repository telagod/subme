...
<svelte:head>
	<title>{$_('nav.quench.announcements', { default: '公告管理' })}</title>
</svelte:head>
...
<div class="space-y-4 px-5 py-5">
	<header class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
		<div>
			<h1 class="text-xl font-semibold tracking-tight text-foreground">
				{$_('nav.quench.announcements', { default: '公告管理' })}
			</h1>
			<p class="text-sm text-muted-foreground">{$_('admin.announcements.description', { default: '发布弹出或静默通知，可选择目标时间窗口。' })}</p>
		</div>
	...
	{#if showEditor}
		<Card class="p-4">
			<div class="mb-3 flex items-center gap-2 text-sm font-semibold">
				...
				{editing ? $_('admin.announcements.editAnnouncement', { default: 'Edit announcement' }) : $_('admin.announcements.createAnnouncement', { default: '创建公告' })}
			</div>
			<div class="grid gap-3 lg:grid-cols-2">
				<label class="space-y-1 text-xs font-medium text-muted-foreground">
					<span>{$_('admin.announcements.columns.title', { default: '标题' })}</span>
					<Input bind:value={form.title} />
				</label>
				<div class="grid gap-3 sm:grid-cols-2">
					<label class="space-y-1 text-xs font-medium text-muted-foreground">
						<span>{$_('admin.announcements.columns.status', { default: '状态' })}</span>
						<NativeSelect bind:value={form.status} options={formStatusOptions} />
					</label>
					<label class="space-y-1 text-xs font-medium text-muted-foreground">
						<span>{$_('admin.announcements.columns.notifyMode', { default: '通知方式' })}</span>
				...
				<label class="space-y-1 text-xs font-medium text-muted-foreground lg:col-span-2">
					<span>{$_('admin.announcements.columns.content', { default: '内容' })}</span>
					<Textarea class="min-h-24" bind:value={form.content} />
				</label>
				<div class="grid gap-3 sm:grid-cols-2">
					<label class="space-y-1 text-xs font-medium text-muted-foreground">
						<span>{$_('admin.announcements.columns.startsAt', { default: '开始于' })}</span>
						<Input type="datetime-local" bind:value={form.starts_at} />
					</label>
					<label class="space-y-1 text-xs font-medium text-muted-foreground">
						<span>{$_('admin.announcements.columns.endsAt', { default: '结束于' })}</span>
						<Input type="datetime-local" bind:value={form.ends_at} />
					</label>
				</div>
				<label class="space-y-1 text-xs font-medium text-muted-foreground">
					<span>{$_('admin.announcements.columns.targeting', { default: '目标规则 JSON' })}</span>
					<Textarea class="min-h-24 font-mono text-xs" bind:value={form.targeting} />
				</label>
			</div>
			<div class="mt-3 flex justify-end gap-2">
				<Button variant="outline" disabled={saving} onclick={() => showEditor = false}>{$_('common.cancel', { default: '取消' })}</Button>
				<Button disabled={saving || !form.title.trim() || !form.content.trim()} onclick={saveAnnouncement}>{$_('common.save', { default: '保存' })}</Button>
			</div>
		</Card>
	{/if}
	...
	<Card class="flex flex-col gap-3 p-3 lg:flex-row lg:items-center lg:justify-between">
		<div class="relative flex-1">
			...
			<Input class="pl-9" placeholder={$_('admin.announcements.searchPlaceholder', { default: '搜索公告' })} bind:value={searchInput} onkeydown={(event) => { if (event.key === 'Enter') resetAndLoad(); }} />
		</div>
		...
		<VirtualTable rows={rows} rowHeight={76} getRowKey={(row) => row.id} loading={loading}>
			{#snippet header()}
				<div class="grid grid-cols-[minmax(260px,1.4fr)_110px_110px_minmax(170px,1fr)_170px_160px] border-b bg-muted/60 px-3 py-2 text-xs font-medium uppercase text-muted-foreground">
					<div>{$_('admin.announcements.columns.title', { default: '标题' })}</div><div>{$_('admin.announcements.columns.status', { default: '状态' })}</div><div>{$_('admin.announcements.columns.notifyMode', { default: 'Notify' })}</div><div>{$_('admin.announcements.columns.targeting', { default: 'Targeting' })}</div><div>{$_('admin.announcements.columns.window', { default: 'Window' })}</div><div class="text-right">{$_('admin.announcements.columns.actions', { default: '操作' })}</div>
				</div>
			{/snippet}
			{#snippet row({ row })}
				<div class="grid min-w-[1080px] grid-cols-[minmax(260px,1.4fr)_110px_110px_minmax(170px,1fr)_170px_160px] items-center border-b px-3 py-3 text-sm" data-testid="admin-announcements-row" data-announcement-id={row.id}>
					...
					<div class="flex justify-end gap-1.5">
						<Button variant="outline" size="sm" disabled={saving} onclick={() => openEdit(row)}>{$_('common.edit', { default: '编辑' })}</Button>
			...
			{/snippet}
			{#snippet empty()}<div class="p-6 text-center text-sm text-muted-foreground">{$_('admin.announcements.empty', { default: '暂无公告' })}</div>{/snippet}
			{#snippet loadingSlot()}<div class="p-4 text-sm text-muted-foreground">{$_('common.loading', { default: '加载中...' })}</div>{/snippet}
		</VirtualTable>
