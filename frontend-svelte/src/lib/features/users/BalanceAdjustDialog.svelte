<script lang="ts">
	...
	async function submit() {
		if (!amount || amount <= 0) {
			showError($_('admin.users.balanceInvalidAmount', { default: '金额必须大于 0' }));
			return;
		}
		submitting = true;
		try {
			await updateUserBalance(userId, amount, operation, notes);
			showSuccess($_('admin.users.balanceAdjusted', { default: '余额已调整' }));
			reset();
			onUpdated();
			onClose();
		} catch (e: unknown) {
			showError((e as Error)?.message || $_('admin.users.balanceFailed', { default: '调整余额失败' }));
		} finally {
			submitting = false;
		}
	}
...
<StandardDialog {open} onOpenChange={(v) => { if (!v) onClose(); }} title={$_('admin.users.adjustBalance', { default: '调整余额' })}
	data-testid="balance-adjust-dialog">
	<div class="flex flex-col gap-4">
		<div class="text-sm text-muted-foreground">
			{$_('admin.users.currentBalance', { default: '当前余额' })}:
			<span class="font-mono font-semibold text-foreground">${fmtBal(currentBalance)}</span>
		</div>

		<div class="flex flex-col gap-1.5">
			<span class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
				{$_('admin.users.operation', { default: '操作' })}
			</span>
			<div class="flex gap-1.5">
				{#each [
					{ value: 'add', label: $_('admin.users.opAdd', { default: '添加' }) },
					{ value: 'subtract', label: $_('admin.users.opSubtract', { default: '扣减' }) },
					{ value: 'set', label: $_('admin.users.opSet', { default: '设置' }) }
				] as op}
		...
		<div class="flex flex-col gap-1.5">
			<span class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
				{$_('admin.users.amount', { default: '金额' })}
			</span>
			<div class="relative">
