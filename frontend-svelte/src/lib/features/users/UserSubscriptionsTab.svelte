<script lang="ts">
	...
	function statusLabel(s: string): string {
		if (s === 'active') return $_('admin.users.subStatusActive', { default: '活跃' });
		if (s === 'expired') return $_('admin.users.subStatusExpired', { default: '已过期' });
		if (s === 'revoked') return $_('admin.users.subStatusRevoked', { default: '已撤销' });
		return s;
	}
...
<div class="flex flex-col gap-2.5">
	...
	{:else if !items.length}
		<div class="py-5 text-center text-sm text-muted-foreground">{$_('admin.users.noSubscriptions', { default: '暂无订阅' })}</div>
	{:else}
		{#each items as sub}
			<Card class="flex flex-col gap-1.5 px-3.5 py-3">
				...
				<div class="flex flex-wrap gap-4 text-xs text-muted-foreground">
					<span>{$_('admin.users.subStart', { default: '开始：' })} {fmt(sub.started_at)}</span>
					{#if sub.expires_at}
						<span>{$_('admin.users.subExpires', { default: '过期：' })} {fmt(sub.expires_at)}</span>
					{:else}
						<span>{$_('admin.users.subPermanent', { default: '永久' })}</span>
					{/if}
				</div>
				{#if sub.mtd_cost != null || sub.price_paid != null}
					<div class="flex flex-wrap gap-4 text-xs text-muted-foreground">
						{#if sub.mtd_cost != null}
							<span>{$_('admin.users.subMtdCost', { default: '本月费用：' })} ${fmtCost(sub.mtd_cost)}</span>
						{/if}
						{#if sub.price_paid != null}
							<span>{$_('admin.users.subPricePaid', { default: '已支付：' })} ${fmtCost(sub.price_paid)}</span>
						{/if}
						{#if sub.remaining_value != null}
							<span>{$_('admin.users.subRemaining', { default: '剩余：' })} ${fmtCost(sub.remaining_value)}</span>
						{/if}
					</div>
				{/if}
			</Card>
